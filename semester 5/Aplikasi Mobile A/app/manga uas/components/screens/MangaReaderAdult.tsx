...
export default function MangaReaderAdult({ route, navigation }) {
  ...
  const fetchChapters = async () => {
    try {
      // 👉 tambahkan contentRating[] lengkap supaya semua chapter muncul
      const res = await fetch(
        `https://api.mangadex.org/chapter?manga=${mangaId}&limit=20&translatedLanguage[]=en&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&contentRating[]=pornographic`
      );
  ...
  const fetchPages = async (chapterId) => {
    ...
    try {
      ...
      const data = await res.json();
      const imgs = data.chapter?.data?.map(
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{title} (Adult)</Text>
