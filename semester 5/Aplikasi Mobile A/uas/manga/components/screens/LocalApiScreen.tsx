...
import config from '../../utils/config'; // path ke utils/config.js
...
export default function LocalApiScreen({ navigation }: any) {
  ...
  useEffect(() => {
    fetch(config.READ) // ambil dari config.js
      .then(res => res.json())
      .then(data => {
        setMangaList(data);
        setLoading(false);
    ...
    <ScrollView style={styles.container}>
      ...
      {mangaList.map((manga, index) => (
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('MangaReader', { title: manga.title, pages: manga.pages })}
        >
