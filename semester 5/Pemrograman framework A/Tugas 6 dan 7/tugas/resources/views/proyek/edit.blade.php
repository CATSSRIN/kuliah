...
<form action="{{ route('proyek.update', $proyek->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <textarea name="deskripsi" class="form-control">{{ old('deskripsi', $proyek->deskripsi) }}</textarea>
    ...
    <div class="mb-3">
        ...
        <select name="mahasiswa_id" class="form-select">
            @foreach ($mahasiswas as $mhs)
                <option value="{{ $mhs->id }}" {{ old('mahasiswa_id', $proyek->mahasiswa_id) == $mhs->id ? 'selected' : '' }}>
                    {{ $mhs->nama }} ({{ $mhs->nim }})
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        @if ($proyek->dokumen)
            <small>Dokumen saat ini: <a href="{{ asset('storage/'.$proyek->dokumen) }}" target="_blank">Lihat Dokumen</a></small>
        @endif
